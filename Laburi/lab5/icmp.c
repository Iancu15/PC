...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	for(int i = 0; i < count; i++) {
		...
		*/
		...
		memcpy(&ip_hdr->daddr, &ip_addr, sizeof(uint32_t));
		ip_hdr->check = 0;
		...
		*/
		...
		struct in_addr a;
       	memcpy(&a.s_addr, &rip_hdr->saddr, sizeof(uint32_t));
		/* TODO 1 afișați informațiile relevante din pachetul reply */
		printf("%zu bytes from %s: icmp_seq=%u ttl=%u time=%.1f ms type=%d code=%d\n",
				pkt.size, inet_ntoa(a), ricmp_hdr->un.echo.sequence,
				rip_hdr->ttl, time_elapsed, ricmp_hdr->type, ricmp_hdr->code);
	}
}
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	for(int i = 0; i < 64; i++) {
		...
		*/
		...
		if (ricmp_hdr->code == 0 && ricmp_hdr->type == 0)
			break;
	}
}
